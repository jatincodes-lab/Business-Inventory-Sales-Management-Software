...
import { InventoryAdjustmentForm, type InventoryOption, type StockOption, type WarehouseOption } from "@/components/inventory-adjustment-form";
import { assertWorkspace, getWorkspaceContext } from "@/lib/supabase/workspace";

function nextNumber(value: string | null | undefined) { const match = value?.match(/(\d+)$/); return `ADJ-${String(match ? Number(match[1]) + 1 : 1).padStart(5, "0")}`; }
...
export default async function NewInventoryAdjustmentPage() {
  ...
  if (allowed !== true) return <div className="rounded-xl border border-[#e2e8f0] bg-white p-8"><h1 className="text-xl font-semibold text-[#0f172a]">New stock adjustment</h1><p className="mt-2 text-sm text-[#64748b]">You do not have permission to create adjustments.</p></div>;
  const [{ data: warehouses, error: warehouseError }, { data: items, error: itemError }, { data: balances, error: balanceError }, { data: latest, error: latestError }] = await Promise.all([
    ...
    context.supabase.from("inventory_adjustments").select("adjustment_number").eq("business_id", context.businessId).order("created_at", { ascending: false }).limit(1).maybeSingle(),
  ]);
  if (warehouseError || itemError || balanceError || latestError) redirect("/auth/error?error=Unable%20to%20load%20adjustment%20data");
  ...
  return <InventoryAdjustmentForm warehouses={(warehouses ?? []) as WarehouseOption[]} items={itemOptions} balances={(balances ?? []) as StockOption[]} suggestedNumber={nextNumber(latest?.adjustment_number)} initialDate={new Date().toISOString().slice(0, 10)} />;
}
