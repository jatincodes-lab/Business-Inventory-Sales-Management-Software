...
begin
  ...
  if (
    select count(*) from jsonb_array_elements(lines) as line(value)
  ) <> (
    select count(distinct line.value ->> 'sales_order_line_id') from jsonb_array_elements(lines) as line(value)
  ) then raise exception 'A sales order line can appear only once per fulfillment'; end if;
