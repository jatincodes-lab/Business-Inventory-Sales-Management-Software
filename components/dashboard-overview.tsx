...
function TrendChart({ points, restricted = false }: { points: Array<{ label: string; value: number }>; restricted?: boolean }) {
  ...
  const gap = points.length > 1 ? Math.min(10, chartWidth / (points.length * 3)) : 0;
  const barWidth = points.length === 1 ? Math.min(72, chartWidth * 0.5) : (chartWidth - gap * (points.length - 1)) / points.length;
  const bars = points.map((point, index) => {
    const barHeight = point.value > 0 ? Math.max((point.value / max) * chartHeight, 2) : 0;
    return { ...point, x: padding.left + (chartWidth - (barWidth * points.length + gap * (points.length - 1))) / 2 + index * (barWidth + gap) + barWidth / 2, y: padding.top + chartHeight - barHeight, barHeight };
  });
  const labels = bars.length <= 4 ? bars : [bars[0], bars[Math.floor((bars.length - 1) / 2)], bars[bars.length - 1]];
  return <div className="relative px-4 pb-3 pt-4"><svg viewBox={`0 0 ${width} ${height}`} className="pointer-events-none h-auto w-full" role="img" aria-label="Sales trend bar chart"><title>Sales trend</title>{[0, 1, 2, 3].map((step) => { const y = padding.top + (chartHeight / 3) * step; return <line key={step} x1={padding.left} x2={padding.left + chartWidth} y1={y} y2={y} stroke="#edf2f4" strokeDasharray="3 5" />; })}<line x1={padding.left} x2={padding.left + chartWidth} y1={padding.top + chartHeight} y2={padding.top + chartHeight} stroke="#dbe2e8" />{bars.map((point) => <rect key={`${point.label}-${point.x}`} x={point.x - barWidth / 2} y={point.y} width={barWidth} height={point.barHeight} rx="3" fill="#00a63e"><title>{`${point.label}: ${money(point.value)}`}</title></rect>)}{labels.map((point) => <text key={`label-${point.label}`} x={point.x} y={height - 8} textAnchor="middle" fill="#64748b" fontSize="11">{point.label}</text>)}</svg><div className="pointer-events-none absolute inset-0" aria-label="Sales values"><div className="relative size-full">{bars.filter((point) => point.barHeight > 0).map((point) => <button key={`tooltip-${point.label}-${point.x}`} type="button" aria-label={`${point.label}: ${money(point.value)}`} className="pointer-events-auto group absolute rounded-md focus:outline-none focus-visible:ring-2 focus-visible:ring-[#00a63e]" style={{ left: `${(point.x / width) * 100}%`, top: `${(point.y / height) * 100}%`, width: `${(barWidth / width) * 100}%`, height: `${(point.barHeight / height) * 100}%` }}><span className={`pointer-events-none absolute left-1/2 z-20 -translate-x-1/2 whitespace-nowrap rounded-md bg-[#0f172a] px-2.5 py-1.5 text-[11px] font-medium text-white opacity-0 shadow-lg transition-opacity group-hover:opacity-100 group-focus:opacity-100 ${point.y < 80 ? "top-full mt-2" : "bottom-full mb-2"}`}>{point.label} · {money(point.value)}</span></button>)}</div></div></div>;
}
